...
class MyWalker extends Walker_Page {
  ...
  // like explode but
  // splitAtLastOccurrence('<li class="page_item page-item-2 page_item_has_children current_page_item"><a href="http://wemag.de/">wemag</a>', 'wemag')
  // returns an array of just 2 elements, not three (as explode), namely
  // array(
  //  [0] = '<li class="page_item page-item-2 page_item_has_children current_page_item"><a href="http://wemag.de/">'
  //  [1] = '</a>
  // )
  //
  ...
  public function start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
    ...
    // disable sublists for some pages
    /*
     * if (!user_has_mouse()) {
     *  if ($page->post_title == "NoSubList1" || 
     *      $page->post_title == "NoSubList2") {
    ...
    // the string 'wemag' actually occurs twice in
    // $newPart = '<li class="page_item page-item-2 page_item_has_children current_page_item"><a href="http://wemag.de/">wemag</a>'
    ...
    /* $specialPageName = "SpceialPageTitle";
     * $resultTitle = 'SpecialPageTitleReplaced';
    ...
    /* 
     * $specialPageName = "OriginalTitle";
     * $resultTitle = "Replace";
     * if ($titleAdded == $specialPageName
     *    && $page->post_parent === get_page_by_title('DesiredParentTitle')->ID) {
     *  $titleAdded = $resultTitle;
     * }
     */
...
// on the home page show 'wemag | description of home page'
// on the other pages show
// 'wemag | title of the page'
function custom_title( $title ) {
  if( empty( $title ) && ( is_home() || is_front_page() ) ) {
    return __( 'wemag', 'theme_domain' ) . ' | ' . get_bloginfo( 'description' );
  }
  return 'wemag |' . $title;
}
